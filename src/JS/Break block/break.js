...
function circle() {
  ...
  if (game && !death) {
    ...
    if (
      circleY + radius > paddleY &&
      circleX + radius > paddleX &&
      circleX - radius < paddleX + 125
    ) {
      speedY *= -1;
    }
    // 벽돌에 튕겼을 때
    for (let i = 0; i < 2; i++) {
      for (let j = 0; j < 4; j++) {
        if (
          circleY - radius < j * 50 + 50 &&
          circleY + radius > j * 50 + 10 &&
          circleX + radius > i * 185 + 10 &&
          circleX - radius < i * 185 + 185
        ) {
